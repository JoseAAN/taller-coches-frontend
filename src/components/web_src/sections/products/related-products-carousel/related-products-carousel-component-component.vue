<template>
    <div class="carousel-container mt-5">
        ...
        <Carousel 
            ...
            :value="products" 
            :numVisible="dynamicNumVisible" 
            :numScroll="1" 
            :responsiveOptions="dynamicResponsiveOptions" 
            :circular="dynamicCircular" 
                ...
                <div class="product-card" @click="goToProduct(property.data.id)">
                    <div class="image-wrapper">
                        <span class="related-badge">
                            <i class="pi pi-tags mr-1" style="font-size: 0.8rem"></i> 
                            {{ property.data.categories && property.data.categories.length ? property.data.categories[0].name : 'Sugerencia' }}
                        </span>
                    ...
                    <div class="product-info">
                        ...
                        <div class="product-desc text-sm mb-3">
        ...
        <div v-else class="text-center py-5 empty-state">
            ...
            <p>No hay productos relacionados por el momento.</p>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        dynamicCircular() {
            return this.products.length > this.dynamicNumVisible;
        },
        dynamicResponsiveOptions() {
            const count = this.products.length;
            return [
                { breakpoint: '1400px', numVisible: Math.min(4, count), numScroll: 1 },
                { breakpoint: '1199px', numVisible: Math.min(3, count), numScroll: 1 },
                { breakpoint: '991px', numVisible: Math.min(2, count), numScroll: 1 },
                { breakpoint: '767px', numVisible: Math.min(1, count), numScroll: 1 }
            ];
        }
    },
    watch: {
    ...
    methods: {
        fetchRelatedProducts() {
            this.loading = true;
            fetch(`${BASE_URL}/v1/products/${this.productId}/related`)
                .then(response => {
                    if (!response.ok) {
...
.related-badge {
    ...
    top: 15px;
    left: 15px;
    background: rgba(82, 177, 85, 0.85);
